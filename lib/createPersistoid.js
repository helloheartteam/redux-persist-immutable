...
function createPersistoid(config) {
  ...
  var getKeys = function getKeys(state) {
    return state && state.keySeq().toArray() || [];
  };

  // update started getting only Immutable Maps as a Reducer Type
  var update = function update(state) {
    // add any changed keys to the queue
    getKeys(state).forEach(function (key) {
      if (!passWhitelistBlacklist(key)) return; // is keyspace ignored? noop
      if ((0, _immutable.is)(lastState.get(key), state.get(key))) return; // value unchanged? noop
    ...
    getKeys(lastState).forEach(function (key) {
